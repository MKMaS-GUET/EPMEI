...
CUDA_VISIBLE_DEVICES=$GPU_ID  python3  run_sc.py  --model_type bertsub  \
    ...
    --seed $seed      \
    --test_file result/ner/$dataset/final_best_result/ent_pred_test_pro.json \
    --use_ner_results \
    --use_typemarker --gcn --n_gcn 2\
    --output_dir result/re/$dataset/best_result_final_t$seed --overwrite_output_dir --focalloss
done;
# Average the scores
