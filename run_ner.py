...
class ACEDatasetNER(Dataset):
    def __init__(self, tokenizer, args=None, evaluate=False, do_test=False):
        ...
        elif args.data_dir.find('scierc_1') != -1:
            self.ner_label_list = ['NIL', 'opinion', 'target']
        ...
        for l_idx, line in enumerate(DATA):
            ...
            for i in range(len(sentences)):
                for j in range(len(sentences[i])):
                    sentences[i][j] = self.get_original_token(sentences[i][j])  # 还原括号
            ...
            words = []
            L = 0
            for i in range(len(sentences)):  # 因为一个sample中可能有多个句子，划分边界
                L += len(sentences[i])
                sentence_boundaries.append(L)
                words += sentences[i]

            tokens = [tokenize_word(w) for w in words]
            subwords = [w for li in tokens for w in li]  # 一个词可能会被分片
            ...
            for n in range(len(subword_sentence_boundaries) - 1):  # 遍历一个sample的所有句子
                sentence_ners = ners[n]  # 第n个句子的entity
                for ner in sentence_ners:
                    if ner[2] == 'target':
                        self.aspect_term += 1
                    elif ner[2] == 'opinion':
                        self.opinion_term += 1
                self.tot_recall += len(sentence_ners)  # 用来记录所有entity的数量
                assert self.aspect_term + self.opinion_term == self.tot_recall
                entity_labels = {}
                for start, end, label in sentence_ners:
                    entity_labels[(token2subword[start], token2subword[end + 1])] = ner_label_map[
                        label]  # entity_labels中的 entity的位置是左闭右开的
                ...
                for entity_start in range(left_context_length,
                                          left_context_length + sentence_length):  # neibor-oriented
                    doc_entity_start = entity_start + doc_offset
                    if doc_entity_start not in subword_start_positions:
                        continue
                    for entity_end in range(entity_start + 1, left_context_length + sentence_length + 1):  # 遍历每个跨度
                        ...
                        if subword2token[doc_entity_end - 1] - subword2token[
                            doc_entity_start] + 1 > self.args.max_mention_ori_length:  # 不能超过最大的跨度大小
                            continue
                        self.all_entity += 1
                ...
                maxR = max(maxR, len(entity_infos))
                dL = self.max_pair_length  # 一组多少个span
                ...
                if not self.args.group_edge:
                    for i in range(0, len(entity_infos), dL):  # 将一组打包

                        examples = entity_infos[i: i + dL]
                        item = {
                            'sentence': target_tokens,  # 句子
                            'examples': examples,  # 所有span
                            'example_index': (l_idx, n),  # (第几个句子，第几个样例)
                            'example_L': len(entity_infos),
                            'adj': data['graph']
                        }
                        self.data.append(item)
    ...
    def __getitem__(self, idx):
        ...
        input_ids += [0] * (self.max_seq_length - len(input_ids))  # 补0至最长序列
        ...
        if self.model_type not in ['bertspan', 'robertaspan', 'albertspan']:
            ...
            if self.model_type.startswith('albert'):
                ...
                input_ids = input_ids + [1] * (len(entry['examples'])) + [0] * (
                            self.max_pair_length - len(entry['examples']))  # 在后面加上128个1
                input_ids = input_ids + [2] * (len(entry['examples'])) + [0] * (
                            self.max_pair_length - len(entry['examples']))  # 在后面加上128个2

            attention_mask = torch.zeros(
                (self.max_entity_length + self.max_seq_length, self.max_entity_length + self.max_seq_length),
                dtype=torch.int64)  # 定向注意力机制
            attention_mask[:L, :L] = 1  # 句子单词之间可见
        ...
        full_attention_mask = [1] * L + [0] * (self.max_seq_length - L) + [0] * (
            self.max_pair_length) * 2  # 句子 + max_pair_lenght *2 因为是一对 标签
        ...
        # 句子可以看见 marker， marker 可以看见marker  ， 但 marker 看不到句子

        labels += [-1] * (num_pair - len(labels))
        mention_pos += [(0, 0)] * (num_pair - len(mention_pos))  # 如果这一组 没有 满128个 则后面的填（0，0）

        te = np.zeros((self.max_seq_length + num_pair * 2, self.max_seq_length + num_pair * 2))
        te[:len(entry['adj'][0]), :len(entry['adj'][0])] = entry['adj']
        te = te.astype(float)
        np.fill_diagonal(te, 1)

        # ====new====
        padded_x = torch.zeros((1, num_pair, 20))
        span_width = [i[2][1] - i[2][0] for i in entry['examples']]
        span_width = torch.tensor(span_width)
        span_width = torch.unsqueeze(span_width, 0)
        sub_max_value = torch.max(span_width).item()
        if sub_max_value >= self.args.num_width_embeddings:
            span_span_width_embedding_over = Embedding(num_embeddings=sub_max_value + 1,
                                                       embedding_dim=self.args.span_width_embedding_dim)
            span_width_embeddings = span_span_width_embedding_over(span_width)
        else:
            span_width_embeddings = self._span_width_embedding(span_width)
        # span_width_embeddings = self._span_width_embedding(span_width)

        padded_x[:, :span_width.size()[1], :] = span_width_embeddings
        span_width_embeddings = padded_x
        span_width_embeddings = torch.squeeze(span_width_embeddings, 0)
        ...
        item = [torch.tensor(input_ids),
                ...
                torch.tensor(span_width_embeddings)
                ]
...
def evaluate(args, model, tokenizer, prefix="", do_test=False):
    ...
    ap_tot_recall = eval_dataset.aspect_term
    op_tot_recall = eval_dataset.opinion_term
    ...
    for example_index, pair_dict in scores.items():
        ...
        for item in no_overlap:
            m2 = item[1]
            pred_ner_label = item[2]
            if pred_ner_label == 'target':
                ap_pre_tot += 1
            elif pred_ner_label == 'opinion':
                op_pre_tot += 1
            tot_pred += 1
    ...
    if ap_pre_tot != 0:
        ap_p = ap_cor / ap_pre_tot
        ap_r = ap_cor / ap_tot_recall
        ap_f1 = 2 * (ap_p * ap_r) / (ap_p + ap_r) if ap_cor > 0 else 0.0
    else:
        ap_p = 0
        ap_r = 0
        ap_f1 = 0
    if op_pre_tot != 0:
        op_p = op_cor / op_pre_tot
        op_r = op_cor / op_tot_recall
        op_f1 = 2 * (op_p * op_r) / (op_p + op_r) if op_cor > 0 else 0.0
    else:
        op_p = 0
        op_r = 0
        op_f1 = 0
    ...
    results = {'f1': f1, 'f1_overlap': f1_tot, 'precision': precision_score, 'recall': recall_score, 'ap_p':ap_p,'ap_r':ap_r,'ap_f1': ap_f1,
               'op_p':op_p,'op_r':op_r,'op_f1': op_f1}

    logger.info("Result: %s", json.dumps(results))

    if args.output_results:
        with open(eval_dataset.file_path) as f:
            DATA = json.load(f)
        if do_test:
        ...
        for l_idx, line in enumerate(DATA):
            data = line
            num_sents = len(data['token']['sentences'])
            predicted_ner = []
...
def main():
    ...
    if args.do_train and args.local_rank in [-1, 0] and args.output_dir.find('test') == -1:
        currenti_file_path = __file__
        file_name = os.path.basename(currenti_file_path)
        create_exp_dir(args.output_dir,
                       scripts_to_save=[file_name, 'transformers/src/transformers/modeling_bert.py',
